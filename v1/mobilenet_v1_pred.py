...
Depthwise Separable convolutions consists in performing the first step in 
a depthwise spatial conv (which acts on each input channel separately).
The `depth_multiplier` argument controls how many output channels are 
generated per input channel in the depthwise step. A depthwise convolution 
block consists of a depthwise conv, batchnorm, relu6, pointwise convolution, 
...
The weights for all 16 models are obtained and translated from Tensorflow 
checkpoints found at the weblink as follows. 

Modify some lines of code to comply with TensorFlow 2.2 and Keras 2.4.3 based 
on the code contributed by Francois Cholett/Somshubra Majumdar
...
from keras.layers import Conv2D, Input, Activation, Dropout, Reshape, BatchNormalization, \
...
class DepthwiseConv2D(Conv2D):
    ...
    """
    # Arguments
        kernel_size: An integer or tuple/list of 2 integers of width and height. 
        strides: An integer or tuple/list of 2 integers
        padding: "valid"` or `"same"` with case-insensitive.
        depth_multiplier: The number of depthwise conv output channels for each 
            input channel. The total number = `filters_in * depth_multiplier`.
        data_format: A string, `channels_last` (default) or `channels_first`.
        activation: If  specify nothing, no activation s applied.
        use_bias: Boolean, whether the layer uses a bias vector.
        depthwise_initializer: Initializer for the depthwise kernel matrix
        bias_initializer: Initializer for the bias vector
        depthwise_regularizer: Being applied to the depthwise kernel matrix
        bias_regularizer: Regularizer function applied to the bias vector
        activity_regularizer: Being applied to the output of the layer. 
        depthwise_constraint: Being applied to the depthwise kernel matrix
        bias_constraint: Constraint function applied to the bias vector
    # Input shape
        4D tensor with shape:`[batch, rows, cols, channels]` if 'channels_last'.
    # Output shape
    ...
    def __init__(self, kernel_size, strides=(1,1), padding='valid', depth_multiplier=1, 
        ...
        super(DepthwiseConv2D, self).__init__(filters=None, kernel_size=kernel_size,
                                              strides=strides, padding=padding,
                                              data_format=data_format, activation=activation,
                                              use_bias=use_bias, bias_regularizer=bias_regularizer,
    ...
    def build(self, input_shape):
        ...
        if input_shape[channel_axis] is None:
            raise ValueError('The channel dimension of the inputs to `DepthwiseConv2D` '
    ...
    def call(self, inputs, training=None):
        outputs = K.depthwise_conv2d(inputs, self.depthwise_kernel, strides=self.strides,
...
def _conv_block(inputs, filters, alpha, kernel=(3,3), strides=(1,1)):
    # Adds an initial conv layer (with batchnorma and relu6).
    """
    # Arguments
        ...
        kernel: An int or tuple/list of 2 int, specifying the 2D conv window.
        strides: An integer or tuple/list of 2 integers
    # Input shape
        4D tensor: (samples, rows, cols, channels) if channels_last.
    # Output shape
        4D tensor: (samples, new_rows, new_cols, filters) if channels_last.
...
def _depthwise_conv_block(inputs, pointwise_conv_filters, alpha,
    ...
    """
    # Arguments
        inputs: Input tensor: (rows, cols, channels) if channels_last
        ...
        depth_multiplier: The number of depthwise conv output channels for each 
            input channel. The total number = `filters_in * depth_multiplier`.
        strides: An integer or tuple/list of 2 integers,
        block_id: Integer, a unique id designating the block number.
    # Input shape
        4D tensor with shape: (batch,rows,cols,channels) if channels_last.
    # Output shape
        4D tensor: (samples, new_rows, new_cols, filters) if channels_last.
...
def MobileNet(input_shape=None, alpha=1.0, depth_multiplier=1, dropout=1e-3, include_top=True, 
              weights='imagenet', input_tensor=None, pooling=None, classes=1000):
    # Instantiate the MobileNet architecture.
    """
    To load a MobileNet model via `load_model`, import the custom objects `relu6` and 
    `DepthwiseConv2D` and pass them to the`custom_objects` parameter.
    model = load_model('mobilenet.h5', custom_objects={
                       'relu6': mobilenet.relu6,
                       'DepthwiseConv2D': mobilenet.DepthwiseConv2D})
    # Arguments: 
        input_shape: optional tuple, `include_top` is False or (224,224,3)
        alpha: controls the width of the network.
            - If `alpha` < 1.0, proportionally decreases filters # in each layer.
            - If `alpha` > 1.0, proportionally increases filters # in each layer.
            - If `alpha` = 1, default filters # from the paper used at each layer.
        depth_multiplier: depth(or resolution) multiplier for depthwise convolution
    ...
    # Raises
        ...
        RuntimeError: run this model with a backend without support separable conv.
    """
    ...
    if weights == 'imagenet':
        if depth_multiplier != 1:
            raise ValueError('If imagenet weights are being loaded, depth'
                             'multiplier must be 1')

        if alpha not in [0.25, 0.50, 0.75, 1.0]:
            raise ValueError('If imagenet weights are being loaded, alpha can'
                             'be one of `0.25`, `0.50`, `0.75` or `1.0` only.')

        if rows != cols or rows not in [128, 160, 192, 224]:
            raise ValueError('If imagenet weights are being loaded, input must'
                             'have a static square shape (one of (128,128),'
                             ' (160,160), (192,192), or (224, 224)). Input shape'
                             ' provided = %s' % (input_shape,))

    if K.image_data_format() != 'channels_last':
        warnings.warn('The MobileNet family of models is only available for channes_last')
    ...
    for r in [128, 160, 192, 224]:
        for a in [0.25, 0.50, 0.75, 1.0]:
            if r == 224:
                ...
                img_path = '/home/mike/Documents/keras_mobilenet/v1/elephant.jpg'
